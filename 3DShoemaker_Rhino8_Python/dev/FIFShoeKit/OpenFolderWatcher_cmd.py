...
def RunCommand(is_interactive):
    """Run the OpenFolderWatcher command."""
    try:
        from plugin.commands.export_commands import OpenFolderWatcher
        cmd = OpenFolderWatcher()
        ...
        Rhino.RhinoApp.WriteLine(
            "[Feet in Focus Shoe Kit] OpenFolderWatcher: {0}".format(ex)
        )
        return 1
