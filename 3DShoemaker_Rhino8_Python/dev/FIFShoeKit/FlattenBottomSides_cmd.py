...
def RunCommand(is_interactive):
    ...
    try:
        ...
        for i in range(go.ObjectCount):
            ...
            if geom and _flatten_geometry(doc, geom, "FlattenedBottomSide_{0}".format(i)):
                count += 1

        doc.Views.Redraw()
        Rhino.RhinoApp.WriteLine("Flattened {0} bottom side(s).".format(count))
        return rc.Result.Success

    except Exception as e:
        Rhino.RhinoApp.WriteLine("Error flattening bottom sides: {0}".format(e))
