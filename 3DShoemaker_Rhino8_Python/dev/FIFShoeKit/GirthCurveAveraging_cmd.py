...
def RunCommand(is_interactive):
    """Run the GirthCurveAveraging command."""
    try:
        from plugin.commands.editing_commands import GirthCurveAveraging
        cmd = GirthCurveAveraging()
        ...
        Rhino.RhinoApp.WriteLine(
            "[Feet in Focus Shoe Kit] GirthCurveAveraging: {0}".format(ex)
        )
        return 1
