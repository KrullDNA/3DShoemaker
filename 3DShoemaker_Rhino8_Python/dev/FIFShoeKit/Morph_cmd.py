# -*- coding: utf-8 -*-
"""Main morph command with FFD, point-to-point, mesh-to-mesh,
twist, and NURBS surface morphing operations.
...
# ---------------------------------------------------------------------------
#  Helper functions (inlined, no plugin imports)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
#  Point-to-point SpaceMorph (inlined)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
#  Morph operations (static helpers)
# ---------------------------------------------------------------------------
...
def _forefoot_twist_morph(geometry, axis_origin, axis_direction,
                          ...
                          end_distance=100.0):
    """Apply a twist deformation to the forefoot region."""
...
def _rearfoot_twist_morph(geometry, axis_origin, axis_direction,
                          angle_radians, start_distance=0.0,
                          end_distance=100.0):
    """Apply a twist deformation to the rearfoot region."""
    axis_line = Rhino.Geometry.Line(
        axis_origin,
        axis_origin - axis_direction * end_distance,
    )
...
def _run_ffd(doc, tol):
    ...
    Rhino.RhinoApp.WriteLine(
        "Pick {0} target control points:".format(len(source))
    )
    target = _pick_points("Target points", len(source))
    if not target or len(target) != len(source):
        Rhino.RhinoApp.WriteLine(
            "[Feet in Focus Shoe Kit] Source/target point count mismatch."
        )
...
def _run_p2p(doc, tol):
    ...
    target = _pick_points(
        "Target points ({0} needed)".format(len(source)), len(source)
    )
    if not target or len(target) != len(source):
        return Rhino.Commands.Result.Failure
...
def _run_twist(doc, forefoot=True):
    ...
    if forefoot:
        ...
        label = "RearFootTwist"
    Rhino.RhinoApp.WriteLine(
        "[Feet in Focus Shoe Kit] {0} morph applied ({1} deg).".format(
            label, angle_deg
        )
    )
    return Rhino.Commands.Result.Success
